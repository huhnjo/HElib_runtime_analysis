...
long testMult(int x, int y, long m, long phi_m, long d, long mvec1, long mvec2, long gens1, long gens2, long ords1, long ords2, long c_m ){
...
// ******************** KeyGen ******************
  ...
  int b = y;	// Value two as integer
   ...
   // With bootstrapping: ctsum = TCEncBootAdd(publicKey, ct1,ct2)
    ...
    cout << " First Mult time : " <<  elapsed_time << endl;    
    ...
    // ******************** Decrypt ******************
    	...
    	vector<long> foo; START COMMENT OUT
	...
	for(int i=0;i<int(ctprod.size());i++){ // START COMMENT OUT
...
long testAdd(int x, int y, long m, long phi_m, long d, long mvec1, long mvec2, long gens1, long gens2, long ords1, long ords2, long c_m ){
...
// ******************** KeyGen ******************
  ...
  int b = y;	// Value two as integer
   ...
   // With bootstrapping: ctsum = TCEncBootAdd(publicKey, ct1,ct2)
    ...
    timer.startMeasuring();
    vector<Ctxt> ctsum = TCEncBootAdd(publicKey, ct1,ct2); // Add ct1 and ct2, no bootstraaping.
    long elapsed_time = timer.endMeasuring();
	...
	// Check if results are correct
	//	cout<<"a+b: "<<decadd<<" = "<<rightadd<<endl;
	cout<<" Addition : a+b: "<<decadd<<" = "<< rightadd <<endl;
...
//{  p, phi(m),    m, d,  m1,  m2,  m3,   g1,    g2,   g3, ord1,ord2,ord3,c_m}
  ...
  //{  2, 60016, 60787, 22, 89, 683,  0,  2050, 58741,    0, 88, 31,   0, 200}
   ...
   /*test Mult with negative inputs */
   
     ofstream runtimeResults;
     //runtimeResults.open ("negativeMult.txt");
   int j = 0;
     ...
     long time;
          ...
          //Test 4 * 4
     time = testMult(a, b, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
     runtimeResults << "m=" << table[j][2] << "," <<  "a=" << a << "," << "b=" << b << "," << time << endl;
     ...
     /* test Add with negative inputs */
     /*runtimeResults.open ("negativeAdd.txt");

     
      //Test 4 * 4
     a = 4;
     b = 4;
     time = testAdd(a, b, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
     runtimeResults << "m=" << table[j][2] << "," <<  "a=" << a << "," << "b=" << b << "," << time << endl;
   ...
   long  time = testAdd(a, b, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
