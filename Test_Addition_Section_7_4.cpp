...
string testAdd(int R, int x, int y, long m, long phi_m, long d, long mvec1, long mvec2, long gens1, long gens2, long ords1, long ords2, long c_m ){
  ...
  int b = y;	// Value two as integer
   ...
   // With bootstrapping: ctsum = TCEncBootAdd(publicKey, ct1,ct2)
    ...
    for(int i = 0; i < R; i++){
      ...
      if(i == 0){
	ctsum = TCEncBootAdd(publicKey, ctsum,ct2); // Add ct1 and ct2, no bootstraaping.
      }else{
	ctsum = TCEncBootAdd(publicKey, ctsum,ct2_new); // Add ct1 and ct2, no bootstraaping.
      }
      long elapsed_time = timer.endMeasuring();

      int b_new = pow(2, i+1);
      cout << "B_NEW " << b_new << endl;
      vector<bool> b_new_1 = IntToTwoCompMin(b_new);
      int nb_new = b_new_1.size();


      	vector<long> m2_new[nb_new];
	for(int i=0;i<nb_new;i++){
		m2_new[i].push_back(b_new_1[i]);
		while(int(m2_new[i].size())<ea.size()){
			m2_new[i].push_back(0);
		}
	}
	...
	for(int i = 0; i < nb_new; i++){
	  Ctxt foox(publicKey);
	  ea.encrypt(foox, publicKey, m2_new[i]);
	  ct2_new.push_back(foox);
	}   
...
//{  p, phi(m),    m, d,  m1,  m2,  m3,   g1,    g2,   g3, ord1,ord2,ord3,c_m}
  ...
  //{  2, 60016, 60787, 22, 89, 683,  0,  2050, 58741,    0, 88, 31,   0, 200}
   ...
   int R = 10;
     ...
     string out = testAdd(R, 128, 1, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );

     cout << out << endl;
     cout << "2 ----------------------------------" << endl;

     out = testAdd(R, 4, 1, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
     cout << "4 -----------------------------------" << endl;


     out = testAdd(R, 8, 1, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
     cout << "8 -----------------------------------" << endl;
     

     out = testAdd(R, 16, 1, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
     cout << "16 -----------------------------------" << endl;

     out = testAdd(R, 128, 1, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
     cout << "128 -----------------------------------" << endl;
