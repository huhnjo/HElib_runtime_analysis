...
long testAdd(int R, int x, int y, long m, long phi_m, long d, long mvec1, long mvec2, long gens1, long gens2, long ords1, long ords2, long c_m ){
...
//{  p, phi(m),    m, d,  m1,  m2,  m3,   g1,    g2,   g3, ord1,ord2,ord3,c_m}
  ...
  //{  2, 60016, 60787, 22, 89, 683,  0,  2050, 58741,    0, 88, 31,   0, 200}
   ...
   /*
     ...
     runtimeResults.open ("addBootstrapRes.txt");
     ...
     for(int i = 1; i < 16; i++){
       ...
       //repeat test 3 times
       for(int c = 0; c < 3; c++){
	 long time = testAdd(R, 128, b, table[j][2], table[j][1], table[j][3], table[j][4], table[j][5], table[j][7], table[j][8], table[j][10], table[j][11], table[j][13] );
